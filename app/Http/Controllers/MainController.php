...
class MainController extends Controller
{
    public function index() {
		$data['soon'] = Package::whereDate('schedule', '<', date('Y-m-d', strtotime('+2 months')))->whereDate('schedule', '>', date('Y-m-d'))->take(4)->get();
		$data['murah'] = Package::whereDate('schedule', '>', date('Y-m-d'))->where('price_quadruple','<',26000000)->where('price_quadruple','>',15000000)->orwhere('price_quadruple','<',2000)->take(9)->get();
